...
namespace F4ConversationCloud.SuperAdmin.Handler
{
    public class HeaderVariableFormatAttribute : ValidationAttribute, IClientModelValidator
    {
        ...
        public HeaderVariableFormatAttribute()
        {
            // Default error message if none is provided
            ErrorMessage = ErrorMessage ?? "Header must contain exactly one variable {{1}} if a variable is used.";
        }
        ...
        protected override ValidationResult IsValid(object value, ValidationContext validationContext)
        {
            ...
            // If header is null/empty/whitespace -> valid (no variable used)
            if (string.IsNullOrWhiteSpace(header))
                return ValidationResult.Success;

            var trimmed = header.Trim();

            // Treat these special minimal tokens as valid (as per your existing behavior)
            if (trimmed == "{}" || trimmed == "{" || trimmed == "}")
                return ValidationResult.Success;

            // Reject malformed curly patterns
            if (header.Contains("{{}}") || header.Contains("{{}") || header.Contains("{}}"))
            {
                return new ValidationResult("Malformed variable pattern. Use exactly {{1}} if a variable is present.");
            }
            ...
            if (matches.Count == 0)
            {
                // No variables present -> valid
                return ValidationResult.Success;
            }

            // If variables exist, enforce: exactly one and it must be {{1}}
            if (matches.Count != 1 || matches[0].Value != "{{1}}")
            {
        ...
        public void AddValidation(ClientModelValidationContext context)
        {
            ...
            context.Attributes["data-val-headervariableformat"] =
                ErrorMessage ?? "Header must contain exactly one variable {{1}} if a variable is used.";
        }
    }
}
